...
{
    public class SentinelZLEMA_v1_0_0 : Indicator
    {
        ...
        private double _alpha;    // 2 / (N + 1)
        private int    _lag;      // (N - 1) / 2

        protected override void OnStateChange()
        {
            if (State == State.SetDefaults)
            {
                Description              = "Sentinel smoother library — Zero-Lag EMA (clean-room). De-lags the input (2·price − price[lag]) and smooths it with an EMA to cut lag. Draws a smoothed line + a Sentinel glass card. A building block, not a Council voter (no State seam).";
                Name                     = "Sentinel ZLEMA v1.0.0";
                ...
                AddPlot(new Stroke(Brushes.SkyBlue, 2), PlotStyle.Line, "ZLEMA");
            }
            else if (State == State.Configure)
            {
                _alpha = 2.0 / (Period + 1);
                _lag   = (int)Math.Ceiling((Period - 1) / 2.0);   // (N-1)/2, rounded up
            }
        ...
        {
            // Clean-room Zero-Lag EMA: EMA of the de-lagged input d = 2·price[0] − price[lag].
            if (CurrentBar < _lag)
            {
                Value[0] = Input[0];   // warmup: seed with raw input until the lag window fills
                return;
            }
            double d = 2.0 * Input[0] - Input[_lag];
            Value[0] = _alpha * d + (1.0 - _alpha) * Value[1];
        ...
        private void RenderCard()
        {
            ...
            _sp.Text("ZLEMA", r.Left + 16f, r.Top, r.Width - 70f, 16f, SentinelSkin.CInk, 11f, true);
        ...
        [Browsable(false)]
        [XmlIgnore]
        public Series<double> ZLEMA => Values[0];

        #region Properties
        [Range(1, int.MaxValue)]
        [NinjaScriptProperty]
        [Display(Name = "Period", Description = "Zero-Lag EMA lookback (N); sets α = 2/(N+1) and lag = (N−1)/2.", Order = 1, GroupName = "Parameters")]
...
namespace NinjaTrader.NinjaScript.Indicators
{
	...
	{
		private Sentinel.Smoothers.SentinelZLEMA_v1_0_0[] cacheSentinelZLEMA_v1_0_0;
		public Sentinel.Smoothers.SentinelZLEMA_v1_0_0 SentinelZLEMA_v1_0_0(int period, bool showCard, SentinelCardCorner cardCorner, bool showIndicatorLabel)
		{
			return SentinelZLEMA_v1_0_0(Input, period, showCard, cardCorner, showIndicatorLabel);
		}

		public Sentinel.Smoothers.SentinelZLEMA_v1_0_0 SentinelZLEMA_v1_0_0(ISeries<double> input, int period, bool showCard, SentinelCardCorner cardCorner, bool showIndicatorLabel)
		{
			if (cacheSentinelZLEMA_v1_0_0 != null)
				for (int idx = 0; idx < cacheSentinelZLEMA_v1_0_0.Length; idx++)
					if (cacheSentinelZLEMA_v1_0_0[idx] != null && cacheSentinelZLEMA_v1_0_0[idx].Period == period && cacheSentinelZLEMA_v1_0_0[idx].ShowCard == showCard && cacheSentinelZLEMA_v1_0_0[idx].CardCorner == cardCorner && cacheSentinelZLEMA_v1_0_0[idx].ShowIndicatorLabel == showIndicatorLabel && cacheSentinelZLEMA_v1_0_0[idx].EqualsInput(input))
						return cacheSentinelZLEMA_v1_0_0[idx];
			return CacheIndicator<Sentinel.Smoothers.SentinelZLEMA_v1_0_0>(new Sentinel.Smoothers.SentinelZLEMA_v1_0_0(){ Period = period, ShowCard = showCard, CardCorner = cardCorner, ShowIndicatorLabel = showIndicatorLabel }, input, ref cacheSentinelZLEMA_v1_0_0);
		}
	}
}
...
namespace NinjaTrader.NinjaScript.MarketAnalyzerColumns
{
	...
	{
		public Indicators.Sentinel.Smoothers.SentinelZLEMA_v1_0_0 SentinelZLEMA_v1_0_0(int period, bool showCard, SentinelCardCorner cardCorner, bool showIndicatorLabel)
		{
			return indicator.SentinelZLEMA_v1_0_0(Input, period, showCard, cardCorner, showIndicatorLabel);
		}

		public Indicators.Sentinel.Smoothers.SentinelZLEMA_v1_0_0 SentinelZLEMA_v1_0_0(ISeries<double> input , int period, bool showCard, SentinelCardCorner cardCorner, bool showIndicatorLabel)
		{
			return indicator.SentinelZLEMA_v1_0_0(input, period, showCard, cardCorner, showIndicatorLabel);
		}
	}
