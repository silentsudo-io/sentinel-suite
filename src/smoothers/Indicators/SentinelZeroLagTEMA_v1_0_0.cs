...
namespace NinjaTrader.NinjaScript.Indicators.Sentinel.Smoothers
{
    public class SentinelZeroLagTEMA_v1_0_0 : Indicator
    {
        ...
        protected override void OnStateChange()
        {
            ...
            {
                if (_sp != null) { try { _sp.Dispose(); } catch (Exception _sx) { SentinelCore.Swallow("SentinelZeroLagTEMA.OnStateChange", _sx); } _sp = null; }
                try { SentinelSkin.CardLayout.Release(this); } catch (Exception _sx) { SentinelCore.Swallow("SentinelZeroLagTEMA.OnStateChange", _sx); }
            }
        }
        ...
        protected override void OnRender(NinjaTrader.Gui.Chart.ChartControl chartControl, NinjaTrader.Gui.Chart.ChartScale chartScale)
        {
            ...
            try { RenderCard(); } catch (Exception _sx) { SentinelCore.Swallow("SentinelZeroLagTEMA.OnRender", _sx); }
            try { _sp.End(); } catch (Exception _sx) { SentinelCore.Swallow("SentinelZeroLagTEMA.OnRender", _sx); }   // ALWAYS runs — a skipped End() would silently kill the card
        }
