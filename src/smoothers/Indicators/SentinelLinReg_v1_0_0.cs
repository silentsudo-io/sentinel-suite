...
{
    public class SentinelLinReg_v1_0_0 : Indicator
    {
        ...
        protected override void OnStateChange()
        {
            ...
            {
                Description              = "Sentinel smoother library — Linear Regression value (clean-room). Least-squares line fitted over the last N inputs, evaluated at the current bar (the regression VALUE, not the slope). Draws a smoothed line + a Sentinel glass card. A building block, not a Council voter (no State seam).";
                Name                     = "Sentinel LinReg v1.0.0";
                ...
                AddPlot(new Stroke(Brushes.SkyBlue, 2), PlotStyle.Line, "LinReg");
            }
        ...
        {
            // Clean-room least-squares linear regression, evaluated at the current bar.
            // Time index t runs oldest→newest: t = 0 … n-1, with y(t) = Input[n-1-t]. Current bar = t = n-1.
            ...
            for (int t = 0; t < n; t++)
            {
                double y = Input[n - 1 - t];   // oldest at t=0, current at t=n-1
                sumT  += t;
                sumY  += y;
                sumTY += t * y;
                sumT2 += (double)t * t;
            ...
            if (denom == 0.0)
            {
                Value[0] = sumY / n;   // degenerate — flat line
                return;
            }

            double slope     = (n * sumTY - sumT * sumY) / denom;
            double intercept = (sumY - slope * sumT) / n;
            Value[0] = intercept + slope * (n - 1);   // regression line value at the current bar
        ...
        private void RenderCard()
        {
            ...
            _sp.Text("LinReg", r.Left + 16f, r.Top, r.Width - 70f, 16f, SentinelSkin.CInk, 11f, true);
        ...
        [Browsable(false)]
        [XmlIgnore]
        public Series<double> LinReg => Values[0];

        #region Properties
        [Range(1, int.MaxValue)]
        [NinjaScriptProperty]
        [Display(Name = "Period", Description = "Regression window (N) — number of inputs fitted.", Order = 1, GroupName = "Parameters")]
...
namespace NinjaTrader.NinjaScript.Indicators
{
	...
	{
		private Sentinel.Smoothers.SentinelLinReg_v1_0_0[] cacheSentinelLinReg_v1_0_0;
		public Sentinel.Smoothers.SentinelLinReg_v1_0_0 SentinelLinReg_v1_0_0(int period, bool showCard, SentinelCardCorner cardCorner, bool showIndicatorLabel)
		{
			return SentinelLinReg_v1_0_0(Input, period, showCard, cardCorner, showIndicatorLabel);
		}

		public Sentinel.Smoothers.SentinelLinReg_v1_0_0 SentinelLinReg_v1_0_0(ISeries<double> input, int period, bool showCard, SentinelCardCorner cardCorner, bool showIndicatorLabel)
		{
			if (cacheSentinelLinReg_v1_0_0 != null)
				for (int idx = 0; idx < cacheSentinelLinReg_v1_0_0.Length; idx++)
					if (cacheSentinelLinReg_v1_0_0[idx] != null && cacheSentinelLinReg_v1_0_0[idx].Period == period && cacheSentinelLinReg_v1_0_0[idx].ShowCard == showCard && cacheSentinelLinReg_v1_0_0[idx].CardCorner == cardCorner && cacheSentinelLinReg_v1_0_0[idx].ShowIndicatorLabel == showIndicatorLabel && cacheSentinelLinReg_v1_0_0[idx].EqualsInput(input))
						return cacheSentinelLinReg_v1_0_0[idx];
			return CacheIndicator<Sentinel.Smoothers.SentinelLinReg_v1_0_0>(new Sentinel.Smoothers.SentinelLinReg_v1_0_0(){ Period = period, ShowCard = showCard, CardCorner = cardCorner, ShowIndicatorLabel = showIndicatorLabel }, input, ref cacheSentinelLinReg_v1_0_0);
		}
	}
}
...
namespace NinjaTrader.NinjaScript.MarketAnalyzerColumns
{
	...
	{
		public Indicators.Sentinel.Smoothers.SentinelLinReg_v1_0_0 SentinelLinReg_v1_0_0(int period, bool showCard, SentinelCardCorner cardCorner, bool showIndicatorLabel)
		{
			return indicator.SentinelLinReg_v1_0_0(Input, period, showCard, cardCorner, showIndicatorLabel);
		}

		public Indicators.Sentinel.Smoothers.SentinelLinReg_v1_0_0 SentinelLinReg_v1_0_0(ISeries<double> input , int period, bool showCard, SentinelCardCorner cardCorner, bool showIndicatorLabel)
		{
			return indicator.SentinelLinReg_v1_0_0(input, period, showCard, cardCorner, showIndicatorLabel);
		}
	}
