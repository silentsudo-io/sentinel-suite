...
{
    public class SentinelHMA_v1_0_0 : Indicator
    {
        ...
        private Series<double>       _diff;   // 2·WMA(⌊N/2⌋) − WMA(N)
        private int                  _half;
        private int                  _sqrt;
        ...
        protected override void OnStateChange()
        {
            ...
            {
                Description              = "Sentinel smoother library — Hull Moving Average (clean-room). Fast, low-lag smoother = WMA(2·WMA(N/2) − WMA(N), √N). Draws a smoothed line + a Sentinel glass card. A building block, not a Council voter (no State seam).";
                Name                     = "Sentinel HMA v1.0.0";
                ...
                AddPlot(new Stroke(Brushes.SkyBlue, 2), PlotStyle.Line, "HMA");
            }
            else if (State == State.Configure)
            {
                _diff = new Series<double>(this);
                _half = Math.Max(1, Period / 2);                       // ⌊N/2⌋
                _sqrt = Math.Max(1, (int)Math.Round(Math.Sqrt(Period))); // round(√N)
            }
        ...
        // Weighted moving average over a series (most-recent input carries the greatest weight).
        private double Wma(ISeries<double> src, int len)
        {
            ...
            for (int i = 0; i < n; i++)
            {
                int w = n - i;         // barsAgo 0 = newest = heaviest weight
                num += w * src[i];
                den += w;
            }
        ...
        {
            // Clean-room Hull MA: WMA( 2·WMA(⌊N/2⌋) − WMA(N), round(√N) ).
            _diff[0] = 2.0 * Wma(Input, _half) - Wma(Input, Period);
            Value[0] = Wma(_diff, _sqrt);
        ...
        private void RenderCard()
        {
            ...
            _sp.Text("HMA", r.Left + 16f, r.Top, r.Width - 70f, 16f, SentinelSkin.CInk, 11f, true);
        ...
        [Browsable(false)]
        [XmlIgnore]
        public Series<double> HMA => Values[0];

        #region Properties
        [Range(1, int.MaxValue)]
        [NinjaScriptProperty]
        [Display(Name = "Period", Description = "Hull MA lookback (N).", Order = 1, GroupName = "Parameters")]
...
namespace NinjaTrader.NinjaScript.Indicators
{
	...
	{
		private Sentinel.Smoothers.SentinelHMA_v1_0_0[] cacheSentinelHMA_v1_0_0;
		public Sentinel.Smoothers.SentinelHMA_v1_0_0 SentinelHMA_v1_0_0(int period, bool showCard, SentinelCardCorner cardCorner, bool showIndicatorLabel)
		{
			return SentinelHMA_v1_0_0(Input, period, showCard, cardCorner, showIndicatorLabel);
		}

		public Sentinel.Smoothers.SentinelHMA_v1_0_0 SentinelHMA_v1_0_0(ISeries<double> input, int period, bool showCard, SentinelCardCorner cardCorner, bool showIndicatorLabel)
		{
			if (cacheSentinelHMA_v1_0_0 != null)
				for (int idx = 0; idx < cacheSentinelHMA_v1_0_0.Length; idx++)
					if (cacheSentinelHMA_v1_0_0[idx] != null && cacheSentinelHMA_v1_0_0[idx].Period == period && cacheSentinelHMA_v1_0_0[idx].ShowCard == showCard && cacheSentinelHMA_v1_0_0[idx].CardCorner == cardCorner && cacheSentinelHMA_v1_0_0[idx].ShowIndicatorLabel == showIndicatorLabel && cacheSentinelHMA_v1_0_0[idx].EqualsInput(input))
						return cacheSentinelHMA_v1_0_0[idx];
			return CacheIndicator<Sentinel.Smoothers.SentinelHMA_v1_0_0>(new Sentinel.Smoothers.SentinelHMA_v1_0_0(){ Period = period, ShowCard = showCard, CardCorner = cardCorner, ShowIndicatorLabel = showIndicatorLabel }, input, ref cacheSentinelHMA_v1_0_0);
		}
	}
}
...
namespace NinjaTrader.NinjaScript.MarketAnalyzerColumns
{
	...
	{
		public Indicators.Sentinel.Smoothers.SentinelHMA_v1_0_0 SentinelHMA_v1_0_0(int period, bool showCard, SentinelCardCorner cardCorner, bool showIndicatorLabel)
		{
			return indicator.SentinelHMA_v1_0_0(Input, period, showCard, cardCorner, showIndicatorLabel);
		}

		public Indicators.Sentinel.Smoothers.SentinelHMA_v1_0_0 SentinelHMA_v1_0_0(ISeries<double> input , int period, bool showCard, SentinelCardCorner cardCorner, bool showIndicatorLabel)
		{
			return indicator.SentinelHMA_v1_0_0(input, period, showCard, cardCorner, showIndicatorLabel);
		}
	}
