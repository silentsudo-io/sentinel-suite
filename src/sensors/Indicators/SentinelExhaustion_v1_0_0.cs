...
namespace NinjaTrader.NinjaScript.Indicators.Sentinel.Sensors
{
	public class SentinelExhaustion_v1_0_0 : Indicator
	{
		...
		protected override void OnStateChange()
		{
			...
			{
				if (_sp != null) { try { _sp.Dispose(); } catch (Exception _sx) { SentinelCore.Swallow("SentinelExhaustion.OnStateChange", _sx); } _sp = null; }
				try { SentinelSkin.CardLayout.Release(this); } catch (Exception _sx) { SentinelCore.Swallow("SentinelExhaustion.OnStateChange", _sx); }
			}
		}
		...
		{
			if (_scope == null) { try { _scope = SentinelCore.ScopeOf(Instrument, BarsPeriod); } catch (Exception _sx) { SentinelCore.Swallow("SentinelExhaustion.Scope", _sx); } }
			return _scope;
		}
		...
		protected override void OnMarketData(NinjaTrader.Data.MarketDataEventArgs marketDataUpdate)
		{
			...
			try { SentinelCore.TouchExhaustionState(Scope()); } catch (Exception _sx) { SentinelCore.Swallow("SentinelExhaustion.OnMarketData", _sx); }
		}
		...
		protected override void OnBarUpdate()
		{
			...
			if (PublishState && Instrument != null && Instrument.MasterInstrument != null)
			{
				...
				catch (Exception _sx) { SentinelCore.Swallow("SentinelExhaustion.OnBarUpdate", _sx); }
			}
		...
		protected override void OnRender(NinjaTrader.Gui.Chart.ChartControl chartControl, NinjaTrader.Gui.Chart.ChartScale chartScale)
		{
			...
			try { if (ShowCard) RenderCard(); } catch (Exception _sx) { SentinelCore.Swallow("SentinelExhaustion.OnRender", _sx); }
			try { _sp.End(); } catch (Exception _sx) { SentinelCore.Swallow("SentinelExhaustion.OnRender", _sx); }
		}
