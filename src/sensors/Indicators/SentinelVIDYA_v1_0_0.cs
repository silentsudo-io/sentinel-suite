...
namespace NinjaTrader.NinjaScript.Indicators.Sentinel.Sensors
{
	public class SentinelVIDYA_v1_0_0 : Indicator
	{
		...
		protected override void OnStateChange()
		{
			...
			{
				if (_sp != null) { try { _sp.Dispose(); } catch (Exception _sx) { SentinelCore.Swallow("SentinelVIDYA.OnStateChange", _sx); } _sp = null; }
				try { SentinelSkin.CardLayout.Release(this); } catch (Exception _sx) { SentinelCore.Swallow("SentinelVIDYA.OnStateChange", _sx); }
			}
		}
		...
		{
			if (_scope == null) { try { _scope = SentinelCore.ScopeOf(Instrument, BarsPeriod); } catch (Exception _sx) { SentinelCore.Swallow("SentinelVIDYA.Scope", _sx); } }
			return _scope;
		}
		...
		protected override void OnMarketData(NinjaTrader.Data.MarketDataEventArgs marketDataUpdate)
		{
			...
			try { SentinelCore.TouchVidyaState(Scope()); } catch (Exception _sx) { SentinelCore.Swallow("SentinelVIDYA.OnMarketData", _sx); }
		}
		...
		protected override void OnBarUpdate()
		{
			...
			if (PublishState && Instrument != null && Instrument.MasterInstrument != null)
			{
				...
				catch (Exception _sx) { SentinelCore.Swallow("SentinelVIDYA.OnBarUpdate", _sx); }
			}

			if (LogChanges && State == State.Realtime && bias != _lastLoggedSig)
			{
				_lastLoggedSig = bias;
				try
				{
		...
		protected override void OnRender(NinjaTrader.Gui.Chart.ChartControl chartControl, NinjaTrader.Gui.Chart.ChartScale chartScale)
		{
			...
			try { if (ShowCard) RenderCard(); } catch (Exception _sx) { SentinelCore.Swallow("SentinelVIDYA.OnRender", _sx); }
			try { _sp.End(); } catch (Exception _sx) { SentinelCore.Swallow("SentinelVIDYA.OnRender", _sx); }
		}
